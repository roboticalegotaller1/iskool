...
export function getFullRaceFeatureSystem(featureId, gender = 'male', hairHex = '#382218', skinHex = '#FCE7DF') {
  let frontSvg = '';
  let backSvg = '';
    ...
    case 'wolf_ears': // Lobo de las Tormentas
      ...
      <g id="race_wolf_ears">
        ...
        <g filter="url(#wolfShadow)">
          <path d="M 314,115 C 302,96 288,68 274,38 C 268,22 266,10 272,8 C 278,8 296,26 318,54 C 334,74 346,98 350,116 C 342,121 326,121 314,115 Z" 
                fill="url(#wolfSlateL)" stroke="#090D1A" stroke-width="3.8" stroke-linejoin="round" />
          <path d="M 272,8 C 280,18 292,36 300,50 C 292,42 282,26 272,8 Z" fill="#020617" />
          <path d="M 284,54 C 280,38 281,28 283,26 C 290,34 304,52 316,72 C 324,86 328,100 328,106 C 318,110 302,100 284,54 Z" 
                fill="#64748B" stroke="#1E293B" stroke-width="1.8" />
          <path d="M 284,104 C 294,96 306,82 310,68 C 314,80 320,88 326,94 C 328,82 332,74 336,62 C 338,74 340,86 342,96 C 334,108 322,116 308,118 C 296,118 286,112 284,104 Z" 
                fill="#F8FAFC" stroke="#0F172A" stroke-width="2" stroke-linejoin="round" />
          <!-- Silver Storm Hoops -->
          <ellipse cx="274" cy="30" rx="3.5" ry="6" fill="none" stroke="#E2E8F0" stroke-width="2.6" transform="rotate(-30 274 30)" />
          <ellipse cx="279" cy="46" rx="3" ry="5" fill="none" stroke="#E2E8F0" stroke-width="2.2" transform="rotate(-30 279 46)" />
        </g>

        <!-- RIGHT WOLF EAR -->
        <g filter="url(#wolfShadow)">
          <path d="M 454,115 C 466,96 480,68 494,38 C 500,22 502,10 496,8 C 490,8 472,26 450,54 C 434,74 422,98 418,116 C 426,121 442,121 454,115 Z" 
                fill="url(#wolfSlateR)" stroke="#090D1A" stroke-width="3.8" stroke-linejoin="round" />
          <path d="M 496,8 C 488,18 476,36 468,50 C 476,42 486,26 496,8 Z" fill="#020617" />
          <path d="M 484,54 C 488,38 487,28 485,26 C 478,34 464,52 452,72 C 444,86 440,100 440,106 C 450,110 466,100 484,54 Z" 
                fill="#64748B" stroke="#1E293B" stroke-width="1.8" />
          <path d="M 484,104 C 474,96 462,82 458,68 C 454,80 448,88 442,94 C 440,82 436,74 432,62 C 430,74 428,86 426,96 C 434,108 446,116 460,118 C 472,118 482,112 484,104 Z" 
                fill="#F8FAFC" stroke="#0F172A" stroke-width="2" stroke-linejoin="round" />
        </g>
      </g>`;
      break;
    ...
    case 'merfolk_fins': // Aletas Acuáticas de Sirena
      ...
      <g id="race_merfolk_fins">
        ...
        <g filter="url(#waterGlow)">
          <path d="M 326,192 C 295,178 262,162 238,140 C 248,168 255,188 244,206 C 260,210 270,220 260,232 C 278,230 300,226 325,232 Z" 
                fill="url(#finCyanGrad)" stroke="#083344" stroke-width="2.8" stroke-linejoin="round" />
          <path d="M 325,202 Q 284,175 240,143" fill="none" stroke="#E0F2FE" stroke-width="2.2" stroke-linecap="round" />
          <path d="M 325,212 Q 286,196 246,207" fill="none" stroke="#E0F2FE" stroke-width="2" stroke-linecap="round" />
          <path d="M 325,222 Q 296,223 263,233" fill="none" stroke="#E0F2FE" stroke-width="1.8" stroke-linecap="round" />
          <circle cx="242" cy="146" r="2.8" fill="#FFFFFF" opacity="0.9" />
        </g>

        <g filter="url(#waterGlow)">
          <path d="M 442,192 C 473,178 506,162 530,140 C 520,168 513,188 524,206 C 508,210 498,220 508,232 C 490,230 468,226 443,232 Z" 
                fill="url(#finCyanGrad)" stroke="#083344" stroke-width="2.8" stroke-linejoin="round" />
          <path d="M 443,202 Q 484,175 528,143" fill="none" stroke="#E0F2FE" stroke-width="2.2" stroke-linecap="round" />
          <path d="M 443,212 Q 482,196 522,207" fill="none" stroke="#E0F2FE" stroke-width="2" stroke-linecap="round" />
          <path d="M 443,222 Q 472,223 505,233" fill="none" stroke="#E0F2FE" stroke-width="1.8" stroke-linecap="round" />
          <circle cx="526" cy="146" r="2.8" fill="#FFFFFF" opacity="0.9" />
        </g>
      </g>`;
      break;
...
async function renderFullSuite() {
  const traits = [
  ...
  for (const trait of traits) {
    for (const g of genders) {
      const { frontSvg, backSvg } = getFullRaceFeatureSystem(trait, g.key, g.hairHex, g.skinHex);
      
      const layers = [];
      if (backSvg) {
        layers.push({ input: Buffer.from(backSvg) });
      }
      layers.push({ input: g.file });
      if (frontSvg) {
        layers.push({ input: Buffer.from(frontSvg) });
      }
      ...
      const cropHeight = trait === 'fairy_wings' ? 560 : 380;
      await sharp(buffer)
        .extract({ left: 160, top: 0, width: 448, height: cropHeight })
        .toFile(`C:/Users/kami-/.gemini/antigravity-ide/brain/c907be6c-9aff-4820-ae93-5384ee91b324/suite_${trait}_${g.key}.png`);
    }
    console.log(`Rendered full suite: ${trait}`);
  }
}

renderFullSuite().catch(console.error);
