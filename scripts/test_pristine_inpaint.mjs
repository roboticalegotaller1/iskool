...
async function testSmoothSweep() {
  const svg = `<svg xmlns="http://www.w3.org/2000/svg" width="170" height="90" viewBox="300 160 170 90">
    <defs>
      <!-- Expressive, natural anime eye sockets with sweeping upper lash arch -->
      <clipPath id="socketL">
  ...
  await sharp('test_clean_face_smooth3.png')
    .composite([{ input: Buffer.from(svg), top: 0, left: 0 }])
    .toFile('test_smooth_sweep.png');

  console.log('Saved test_smooth_sweep.png');
}
