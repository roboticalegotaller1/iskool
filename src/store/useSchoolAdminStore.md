...
archivo_origen: "src/store/useSchoolAdminStore.ts"
ultima_sincronizacion: "2026-06-21T05:43:37.627Z"
---

# useSchoolAdminStore - Control Escolar, Debounce y Persistencia

## 🏫 Resumen de Store de Administración Escolar y Debounce

Este almacén administra configuraciones del administrador escolar, materias, calendarios, grupos y asistencias.

### ⚡ Características Críticas Implementadas:
- **Patrón Debounce en Configuraciones:**
  - `saveSchoolSettings`: Almacena de forma instantánea en el estado Zustand para mantener la interfaz de usuario fluida, pero pospone y agrupa el envío de persistencia (`upsert`) en Supabase con un retraso (debounce) de 1000ms tras dejar de escribir.
- **Persistencia Real de Mutaciones RLS:**
  - `deleteGroup`: Ejecuta el borrado asíncrono de grupos directamente en Supabase, aplicando el `set` local solo si la respuesta es exitosa.
  - `deleteSchedule`: Borrado asíncrono del horario en `class_schedules` en la base de datos de producción.

### 🧠 Interfaz del Estado y Acciones (`SchoolAdminStoreState`)
