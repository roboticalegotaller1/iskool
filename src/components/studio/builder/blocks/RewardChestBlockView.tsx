...
export const RewardChestBlockView: React.FC<Props> = ({ block }) => {
  ...
  const rarities = [
    ...
    { id: 'epic', name: 'Épico (Esmeralda)', color: 'border-emerald-400 text-emerald-600 dark:text-emerald-400 bg-emerald-50 dark:bg-emerald-950' },
  ...
  return (
    <div className="space-y-4">
          ...
          {rarities.map((r) => (
            <button
              ...
              className={`p-2 rounded-xl text-xs font-black border transition-all text-center cursor-pointer ${
                chestRarity === r.id
                  ? `${r.color} ring-2 ring-emerald-500 shadow-md`
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        <div className="p-3 bg-emerald-50/60 dark:bg-emerald-950/30 rounded-2xl border border-emerald-200/50 dark:border-emerald-800/40 space-y-1.5">
          ...
          <div className="flex items-center gap-2">
            ...
            />
            <span className="text-sm font-black text-emerald-700 dark:text-emerald-300 w-16 text-right">
              +{xpAmount} XP
            </span>
          </div>
        </div>
