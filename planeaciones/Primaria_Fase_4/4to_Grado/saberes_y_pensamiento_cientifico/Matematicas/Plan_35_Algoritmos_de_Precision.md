---
tags: [iskool, planeacion_nem, segundo_cerebro, campo_saberes_y_pensamiento_cientifico, grado_4to_grado, materia_matematicas, fase4_primaria]
campo_formativo: "Saberes y Pensamiento Científico"
materia: "Matemáticas"
grado: "4º de Primaria"
nivel: "Primaria (Fase 4)"
tema: "Multiplicación de 3x2 cifras y división con cociente y residuo"
docente: "Prof. Israel López Ángeles"
fecha_creacion: "18 de agosto de 2026"T18:46:24.068Z"
---

# Algoritmos de Precisión: Multiplicación de 3 por 2 Cifras y Divisiones con Residuo

> [!INFO] **Ficha Técnica NEM 2024 (Fase 4)**
> - **Docente Titular:** Prof. Israel López Ángeles
> - **Nivel / Fase:** 4º de Primaria • Fase 4 (Primaria)
> - **Campo Formativo:** Saberes y Pensamiento Científico
> - **Asignatura / Área:** Matemáticas
> - **Duración Estimada:** 2 sesiones de 50 minutos (Total: 100 min)
> - **Ejes Articuladores:** Pensamiento Crítico • Inclusión

---

## 🎯 I. Proceso de Desarrollo de Aprendizaje (PDA Oficial SEP)

> **Fase 4 (4º Primaria) - Resuelve multiplicaciones de tres por dos cifras con algoritmo convencional y descomposiciones. Resuelve divisiones de tres cifras entre una o dos cifras reconociendo cociente y residuo. Calcula mentalmente dobles, triples y mitades.**

---

## ❓ II. Preguntas Detonadoras para el Salón (Conflicto Cognitivo y Apertura)

1. **¿Cómo se suman los productos parciales al multiplicar 345 x 28?**

2. **¿Qué significa el residuo cuando repartimos 125 libros en cajas de 10?**

3. **¿Por qué el residuo siempre debe ser menor que el divisor?**

4. **¿Cómo calculamos mentalmente el triple de 35 o la mitad de 96?**

---

## ⏱️ III. Secuencia Didáctica (Dosificación en Bloques de 50 minutos)

### 📌 Inicio (10 minutos)
Reparto de canicas con residuo sobrante. Elementos de la división formal.

### 🔬 Desarrollo (30 minutos)
Taller de Algoritmos: Multiplicación de 3x2 cifras por descomposición y algoritmo estándar. Divisiones con comprobación: Dividendo = (Divisor x Cociente) + Residuo.

### 💡 Cierre (10 minutos)
Carrera de cálculo mental de dobles y mitades. Verificación de residuo.

---

## 📋 IV. Evaluación Formativa y Rúbrica Analítica

### Criterios de Evaluación y Niveles de Logro
• **Criterio 1:** Ejecución correcta del algoritmo de multiplicación de 3 por 2 cifras
  - *Sobresaliente (3.5 - 3 pts):* Demuestra dominio integral y autónomo.
  - *Satisfactorio (2.5 - 2 pts):* Aplica los conceptos con orientación básica.
  - *En Proceso (1.5 - 1 pt):* Requiere mediación y acompañamiento docente.

• **Criterio 2:** Resolución de divisiones identificando cociente y residuo
  - *Sobresaliente (3.5 - 3 pts):* Demuestra dominio integral y autónomo.
  - *Satisfactorio (2.5 - 2 pts):* Aplica los conceptos con orientación básica.
  - *En Proceso (1.5 - 1 pt):* Requiere mediación y acompañamiento docente.

• **Criterio 3:** Comprobación formal de operaciones y agilidad en cálculo mental
  - *Sobresaliente (3.5 - 3 pts):* Demuestra dominio integral y autónomo.
  - *Satisfactorio (2.5 - 2 pts):* Aplica los conceptos con orientación básica.
  - *En Proceso (1.5 - 1 pt):* Requiere mediación y acompañamiento docente.

---

## 📦 V. Materiales, Recursos y Evidencias Entregables

### Materiales y Recursos Didácticos
Hojas cuadriculadas, tarjetas de problemas de logística.

### Evidencia de Aprendizaje Entregable
**Problemario de Multiplicaciones de 3x2 Cifras y Divisiones Comprobadas.**

---

## 🔗 VI. Conexiones en el Grafo del Segundo Cerebro (Obsidian Wikilinks)
- [[00_Indice_Maestro_Primaria_Fase4_NEM|Índice Maestro de Primaria Fase 4]]
- [[../../00_Indice_Maestro_Primaria_Fase4_NEM|MOC Segundo Cerebro ISkool]]
