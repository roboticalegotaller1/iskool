---
tags: [iskool, planeacion_nem, segundo_cerebro, campo_saberes_y_pensamiento_cientifico, grado_4to_grado, materia_matematicas, fase4_primaria]
campo_formativo: "Saberes y Pensamiento Científico"
materia: "Matemáticas"
grado: "4º de Primaria"
nivel: "Primaria (Fase 4)"
tema: "Cálculo de perímetro y área en retículas cuadradas"
docente: "Prof. Israel López Ángeles"
fecha_creacion: "2026-08-18T18:46:24.074Z"
---

# Diseñadores de Planos: Perímetro (Contorno) vs Área (Superficie) en Cuadrículas

> [!INFO] **Ficha Técnica NEM 2024 (Fase 4)**
> - **Docente Titular:** Prof. Israel López Ángeles
> - **Nivel / Fase:** 4º de Primaria • Fase 4 (Primaria)
> - **Campo Formativo:** Saberes y Pensamiento Científico
> - **Asignatura / Área:** Matemáticas
> - **Duración Estimada:** 2 sesiones de 50 minutos (Total: 100 min)
> - **Ejes Articuladores:** Pensamiento Crítico • Artes y Experiencias Estéticas

---

## 🎯 I. Proceso de Desarrollo de Aprendizaje (PDA Oficial SEP)

> **Fase 4 (4º Primaria) - Distingue contorno (perímetro como suma de lados) y superficie (área como medida de la superficie). Estima y compara áreas con retículas de cuadrados y unidades no convencionales.**

---

## ❓ II. Preguntas Detonadoras para el Salón (Conflicto Cognitivo y Apertura)

1. **¿Por qué el perímetro es la cerca (cm/m) y el área es el piso cubierto (cm²/m²)?**

2. **¿Cómo calculamos el área sumando cuadritos enteros y medios cuadritos?**

3. **¿Pueden dos figuras tener igual perímetro pero diferente área?**

4. **¿Cómo calculamos la superficie de un jardín escolar?**

---

## ⏱️ III. Secuencia Didáctica (Dosificación en Bloques de 50 minutos)

### 📌 Inicio (10 minutos)
Demostración del marco de madera (perímetro) vs lámina que lo cubre (área).

### 🔬 Desarrollo (30 minutos)
Dibujar el plano a escala de una casa escolar en cuadrícula de 1 cm², calculando perímetros sumando longitudes y áreas contando unidades cuadradas y fracciones de cuadrito.

### 💡 Cierre (10 minutos)
Comparación de planos y verificación de que figuras distintas pueden compartir igual área.

---

## 📋 IV. Evaluación Formativa y Rúbrica Analítica

### Criterios de Evaluación y Niveles de Logro
• **Criterio 1:** Diferenciación conceptual y práctica entre perímetro (longitud) y área (superficie)
  - *Sobresaliente (3.5 - 3 pts):* Demuestra dominio integral y autónomo.
  - *Satisfactorio (2.5 - 2 pts):* Aplica los conceptos con orientación básica.
  - *En Proceso (1.5 - 1 pt):* Requiere mediación y acompañamiento docente.

• **Criterio 2:** Cálculo preciso de áreas en retículas considerando mitades de cuadrito
  - *Sobresaliente (3.5 - 3 pts):* Demuestra dominio integral y autónomo.
  - *Satisfactorio (2.5 - 2 pts):* Aplica los conceptos con orientación básica.
  - *En Proceso (1.5 - 1 pt):* Requiere mediación y acompañamiento docente.

• **Criterio 3:** Uso adecuado de unidades lineales y cuadradas
  - *Sobresaliente (3.5 - 3 pts):* Demuestra dominio integral y autónomo.
  - *Satisfactorio (2.5 - 2 pts):* Aplica los conceptos con orientación básica.
  - *En Proceso (1.5 - 1 pt):* Requiere mediación y acompañamiento docente.

---

## 📦 V. Materiales, Recursos y Evidencias Entregables

### Materiales y Recursos Didácticos
Hojas cuadriculadas de 1 cm², colores, reglas graduadas.

### Evidencia de Aprendizaje Entregable
**Plano Arquitectónico Escolar con Cálculo de Perímetro y Área.**

---

## 🔗 VI. Conexiones en el Grafo del Segundo Cerebro (Obsidian Wikilinks)
- [[00_Indice_Maestro_Primaria_Fase4_NEM|Índice Maestro de Primaria Fase 4]]
- [[../../00_Indice_Maestro_Primaria_Fase4_NEM|MOC Segundo Cerebro ISkool]]
