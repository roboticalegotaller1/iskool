---
tags: [iskool, planeacion_nem, segundo_cerebro, campo_saberes_y_pensamiento_cientifico, grado_2do_grado, materia_matematicas, fase6_secundaria]
campo_formativo: "Saberes y Pensamiento Científico"
materia: "Matemáticas"
grado: "2º de Secundaria"
nivel: "Secundaria (Fase 6)"
tema: "Construcción y propiedades de las figuras planas, cuerpos, circunferencia, círculo y esfera"
docente: "Prof. Israel López Ángeles"
fecha_creacion: "18 de agosto de 2026"T05:03:06.921Z"
---

# Geometría del Espacio: De los Polígonos Regulares a la Esfera Terrestre

> [!INFO] **Ficha Técnica NEM 2022**
> - **Docente Titular:** Prof. Israel López Ángeles
> - **Nivel / Fase:** 2º de Secundaria • Fase 6
> - **Campo Formativo:** Saberes y Pensamiento Científico
> - **Asignatura:** Matemáticas
> - **Duración Estimada:** 2 sesiones de 50 minutos (Total: 100 min)
> - **Ejes Articuladores:** Pensamiento Crítico • Artes y Experiencias Estéticas

---

## 🎯 I. Proceso de Desarrollo de Aprendizaje (PDA)

> **Fase 6 (2º Secundaria) - Construye con regla y compás polígonos regulares con información diversa, e identifica las relaciones entre los ángulos inscritos y centrales en la circunferencia, deduciendo las fórmulas de perímetro y área.**

---

## ❓ II. Preguntas Detonadoras para el Salón (Apertura y Conflicto Cognitivo)

1. **¿Por qué el número Pi ($\pi \approx 3.1416$) aparece siempre que dividimos el perímetro de cualquier círculo entre su diámetro?**

2. **¿Qué relación existe entre la medida de un ángulo central y un ángulo inscrito que subtienden el mismo arco?**

3. **¿Cómo construimos un hexágono o un octágono regular perfecto usando únicamente compás y regla no graduada?**

---

## ⏱️ III. Secuencia Didáctica (Dosificación en Bloques de 50 minutos)

### 📌 Inicio (10 minutos)
⏱️ SESIÓN 1 (50 min) — INICIO (10 min):
1. Experimento con tapas circulares de diferentes tamaños e hilo para medir perímetro y diámetro.
2. Pregunta detonadora: "¿Por qué el cociente $P / D$ siempre da 3.14 independientemente del tamaño del objeto?".
3. Definición de radio, cuerda, secante, tangente y arco.

### 🔬 Desarrollo (30 minutos)
⏱️ SESIÓN 1 — DESARROLLO (30 min):
1. Taller de trazo geométrico con regla y compás: construcción de polígonos regulares inscritos (triángulo equilátero, cuadrado, hexágono regular).
2. Demostración del Teorema del Ángulo Inscrito (el ángulo inscrito mide exactamente la mitad del ángulo central correspondiente: $\theta_{\text{ins}} = \theta_{\text{cen}} / 2$).
3. Cálculo de áreas sombreadas y coronas circulares en aplicaciones de diseño arquitectónico.

### 💡 Cierre (10 minutos)
⏱️ SESIÓN 1 — CIERRE (10 min):
1. Galería de mandalas y polígonos regulares trazados con precisión.
2. Metacognición sobre la belleza de la exactitud geométrica.
3. Entrega de evidencia: Lámina de trazo con demostración de ángulos.

---

## 📋 IV. Evaluación Formativa y Rúbrica Analítica

📋 RÚBRICA FORMATIVA ANALÍTICA (3 Criterios):
• Criterio 1: Precisión en el manejo de regla y compás.
• Criterio 2: Demostración formal del ángulo inscrito y central.
• Criterio 3: Cálculo correcto de áreas y perímetros de figuras compuestas.

---

## 📦 V. Materiales, Recursos y Evidencias Entregables

### Materiales y Recursos Didácticos
Compás metálico de precisión, reglas graduadas, transportador de 360°, papel marquilla.

### Evidencia Entregable de la Clase (Producto Tangible)
> 📄 **Lámina de Trazo Geométrico "Geometría Sagrada y Circunferencia" con demostraciones angulares.**

---

## 🔗 Nodos Relacionados y Conexiones en el Segundo Cerebro
- [[00_Indice_Maestro_Secundaria_NEM|Índice Maestro de Secundaria]]
- Tag: #saberes_y_pensamiento_cientifico | #matematicas | #2do_grado
